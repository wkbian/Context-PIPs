...
def main(
    ...
    model_name += "_%s" % lrn
    model_name += "_%d" % max_iters
    if use_augs:
        model_name += "_A"
    model_name += "_%s" % exp_name
    import datetime
    model_date = datetime.datetime.now().strftime('%Y%m%d_%H:%M:%S')
    ...
    train_dataloader_ = DataLoader(
        ...
        drop_last=True)
    train_iterloader = iter(train_dataloader_)
    ...
    if val_freq > 0:
        ...
        val_dataloader_ = DataLoader(
            ...
            num_workers=4,
            drop_last=False)
        val_iterloader = iter(val_dataloader_)
    ...
    if init_dir:
        if load_step and load_optimizer:
            if use_scheduler:
                global_step = saverloader.load(init_dir, model.module, optimizer, scheduler, ignore_load=ignore_load)
            else:
    ...
    while global_step < max_iters:
        ...
        if np.mod(global_step, save_freq)==0:
            if use_scheduler:
                saverloader.save(ckpt_dir, optimizer, model.module, global_step, scheduler, keep_latest=keep_latest)
            else:
