...
from .mlpmixer_fe import SimplifiedFeatureEnhancer, fmaps_sample
...
class CorrBlock:
    ...
    def sample_x10(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            centroid_lvl = coords.unsqueeze(3).repeat(1, 1, 1, T, 1).reshape(B*S*N*T, 1, 1, 2) / 2**i
            delta_lvl = delta.view(1, 2*r+1, 2*r+1, 2)
            coords_lvl = centroid_lvl + delta_lvl

            corrs = bilinear_sampler(corrs.reshape(B*S*N*T, 1, H, W), coords_lvl)
            corrs = corrs.view(B, S, N*T, -1)
            out_pyramid.append(corrs)

        out = torch.cat(out_pyramid, dim=-1) # B, S, N*9, LRR*2
        return out.contiguous().float()
...
class Pips(nn.Module):
    def __init__(self, S=8, stride=8):
        ...
        self.corr_levels = 4
        self.corr_radius = 3
        self.corr_size = self.corr_radius * 2 + 1
        
        self.fnet = BasicEncoder(output_dim=self.latent_dim, norm_fn='instance', dropout=0, stride=stride)

        self.num_samples = 9
        self.sample_corr_levels = 4
        self.sample_corr_radius = 2
        self.sample_corr_size = self.sample_corr_radius * 2 + 1
        self.query_proj = nn.Linear(self.sample_corr_levels * self.sample_corr_size * self.sample_corr_size, self.latent_dim)
        self.query_encoder = nn.Sequential(
            nn.Linear(self.latent_dim, self.latent_dim * 4), nn.ReLU(True),
            nn.Linear(self.latent_dim * 4, self.latent_dim * 4), nn.ReLU(True),
            nn.Linear(self.latent_dim * 4, self.latent_dim)
        )
        self.sampling_offsets = nn.Linear(self.latent_dim, self.num_samples * 2)

        self.fcorrs_dim = self.corr_levels * self.corr_size * self.corr_size
        self.fcorrs_mixer = nn.Linear(self.fcorrs_dim * (self.num_samples + 1) + self.latent_dim, self.fcorrs_dim * (self.num_samples + 1) // 4)
        self.fcorrs_offsets_mixer = nn.Linear(self.fcorrs_dim * (self.num_samples + 1) // 4 + self.latent_dim, self.fcorrs_dim)
        ...
        self.fcorrs_adapter = nn.Linear(self.corr_levels * ((2 * self.corr_radius + 1) ** 2), self.latent_dim)

        self.mlpmixer_fe = SimplifiedFeatureEnhancer(depth=1)

    def forward(self, xys, rgbs, coords_init=None, feat_init=None, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, return_feat=False, is_train=False, feat_x9_init=None, query_init=None):
        ...
        if feat_init is None:
            ...
            ffeat_x9 = []
            ffeat_ = rearrange(ffeat, 'B N C -> B 1 N C')
            ffcorr_fn = CorrBlock(fmaps[:, :1], num_levels=self.sample_corr_levels, radius=self.sample_corr_radius)
            ffcorr_fn.corr(ffeat_)
            ffcorr = ffcorr_fn.sample(coords[:, :1]) # B 1 N C=r*r*4
            # query = torch.cat([ffeat_, ffcorr], -1).squeeze(1) # B N C
            query = ffcorr.squeeze(1) # B N C

            query = self.query_proj(query) # B N C
            query = self.query_encoder(query) # B N C=128
            offsets = self.sampling_offsets(query) # B N C=16
            offsets = rearrange(offsets, 'B N (P C) -> B N P C', C=2)
            offsets = F.tanh(offsets) * 2.0

            for i in range(self.num_samples):
                ffeat_x9.append(utils.samp.bilinear_sample2d(fmaps[:,0], coords[:,0,:,0] + offsets[:, :, i, 0], coords[:,0,:,1] + offsets[:, :, i, 1]).permute(0, 2, 1).unsqueeze(2)) # B, N, 1, C
            ffeat_x9 = torch.cat(ffeat_x9, 2) # B N T=9 C
            # ffeat_x9 = rearrange(ffeat_x9, 'B N T C -> B (N T) C')
        else:
            ffeat = feat_init
            ffeat_x9 = feat_x9_init
            query = query_init

        ffeats = ffeat.unsqueeze(1).repeat(1, S, 1, 1) # B, S, N, C
        ffeats_x9 = ffeat_x9.unsqueeze(1).repeat(1, S, 1, 1, 1) # B, S, N, T, C
        ...
        query_ = query.unsqueeze(1).repeat(1, S, 1, 1)

        for itr in range(iters):
            coords = coords.detach()

            # fcorr_fn.corr(ffeats)
            ffeats_x10 = torch.cat([ffeats.unsqueeze(-2), ffeats_x9], dim=-2)
            ffeats_x10 = rearrange(ffeats_x10, 'B S N T C -> B S (N T) C')
            fcorr_fn.corr(ffeats_x10)
            ...
            fcorrs = fcorr_fn.sample_x10(coords) # B, S, NT, LRR
            fcorrs = rearrange(fcorrs, 'B S (N T) C -> B S N (T C)', T=10)
            fcorrs = self.fcorrs_mixer(torch.cat([fcorrs, query_], -1)) # B, S, N, C=490
            fcorrs = self.fcorrs_offsets_mixer(torch.cat([fcorrs, query_], -1)) # B, S, N, LRR
            LRR = fcorrs.shape[3]

            fmaps_5x5 = fmaps_sample(coords, fmaps) # B S N H5 W5 C
            weight = self.weight(ffeats)
            fcorrs_ = self.fcorrs_adapter(fcorrs)
            ffeats = ffeats + self.mlpmixer_fe(fcorrs_, fmaps_5x5) * weight
        ...
        if trajs_g is not None:
            ...
            losses = (seq_loss, vis_loss)
        else:
            losses = None

        if return_feat:
            return coord_predictions, coord_predictions2, vis_e, ffeat, losses, ffeat_x9, query
        else:
